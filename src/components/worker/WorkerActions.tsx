...
export function WorkerActions({ worker }: { worker: Worker }) {
  ...
  return (
    <div className="flex shrink-0 flex-wrap items-center justify-end gap-2 sm:flex-nowrap">
      {["draft", "submitted", "reviewing", "approved", "published", "rejected"].includes(worker.status) && (
        <Link
          ...
          title={
            worker.status === "published" ? "마켓에서 보기" :
            worker.status === "draft" ? "초안 테스트" :
            worker.status === "rejected" ? "작성자 테스트" :
            "미리보기 / 실행"
          }
        >
